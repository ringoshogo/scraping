...
def _get_item_detail_info(driver, item, category, result_list):
    ...
    # 商品名
    item['item_name'] = _get_item_detail(lambda: soup.select("#productTitle")[0].text.strip(), category, item['item_name'], "item_name", item['item_name'])
    # 商品価格
    price = _get_item_detail(lambda: soup.select("#priceblock_ourprice")[0].text.strip(), category, item['item_name'], "price", "-")
    if price == "-":
        price = _get_item_detail(lambda: soup.select("#priceblock_dealprice")[0].text.strip(), category, item['item_name'], "price", "-")
    # プライムか否か
    is_prime_elems = _get_item_detail(lambda: soup.select("#priceBadging_feature_div > i > i"), category, item['item_name'], "is_prime", 0)
    is_prime = 1 if len(is_prime_elems) > 0 else 0
    # 在庫
    availability = _get_item_detail(lambda: soup.select("#availability > span")[0].text.strip(), category, item['item_name'], "availability", "在庫無し。")
    availability = 1 if "在庫あり。" == availability else 0
    # お届け日
    delivery = _get_item_detail(lambda: soup.select("#ddmDeliveryMessage > b")[0].text.strip(), category, item['item_name'], "delivery", "-")
    # ASIN
    asin = _get_item_detail(lambda: soup.select("#productDetails_detailBullets_sections1 > tbody > tr:nth-child(1) > td")[0].text.strip(), category, item['item_name'], "ASIN", "-")
